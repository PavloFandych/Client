...
public final class DataFinderBundesLiga1 extends DataFinder {
    ...
    protected final String getUrl() {
        try {
            ...
            credentials.load(DataFinderBundesLiga1.class.getClassLoader().getResourceAsStream("credentials.properties"));

            final String api = credentials.getProperty("apiBUNDESLIGA_1");
    ...
    public List<Result> findResults() {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                final Team homeTeam = getGermanTeamDAO().findByTeamName((String) item.get(Constants.HOME_TEAM_NAME));
                final Team awayTeam = getGermanTeamDAO().findByTeamName((String) item.get(Constants.AWAY_TEAM_NAME));
