...
public final class DataFinderEnglishPremierLeague extends DataFinder {
    ...
    protected final String getUrl() {
        try {
            ...
            credentials.load(DataFinderEnglishPremierLeague.class.getClassLoader().getResourceAsStream("credentials.properties"));

            final String api = credentials.getProperty("apiENG_PREM_LEAGUE");
    ...
    public List<Result> findResults() {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                final Team homeTeam = getEnglishTeamDAO().findByTeamName((String) item.get(Constants.HOME_TEAM_NAME));
                final Team awayTeam = getEnglishTeamDAO().findByTeamName((String) item.get(Constants.AWAY_TEAM_NAME));
