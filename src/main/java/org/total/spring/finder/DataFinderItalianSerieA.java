...
public final class DataFinderItalianSerieA extends DataFinder {
    ...
    protected String getUrl() {
        try {
            ...
            credentials.load(DataFinderItalianSerieA.class.getClassLoader().getResourceAsStream("credentials.properties"));

            final String api = credentials.getProperty("apiITA_SERIA_A");
    ...
    public List<Result> findResults() {
        ...
        try {
            ...
            final Tournament tournament = getTournamentDAO().fetchTournamentByTournamentCode(TournamentCode.ITA_SERIA_A.name());

            while (iterator.hasNext()) {
                final JSONObject item = iterator.next();
                final JSONObject result = (JSONObject) item.get("result");

                final Team homeTeam = getItalianTeamDAO().findByTeamName((String) item.get(Constants.HOME_TEAM_NAME));
                final Team awayTeam = getItalianTeamDAO().findByTeamName((String) item.get(Constants.AWAY_TEAM_NAME));
