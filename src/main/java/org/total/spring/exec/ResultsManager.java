    ...
    public static void main(String[] args) {
        if (args != null && args.length > 0) {
            try {
                ...
                final CachedTeamsListMaster cachedTeamsListMaster = (CachedTeamsListMaster) context
                        .getBean("cachedTeamsListMaster");

                final Properties credentials = new Properties();
                credentials.load(ResultsManager.class.getClassLoader().getResourceAsStream("credentials.properties"));
                final String seasonCode = credentials.getProperty("currentSeasonCode");
                ...
                final String duration = String.format("%d min, %d sec", TimeUnit.NANOSECONDS.toMinutes((end - start)),
                        TimeUnit.NANOSECONDS.toSeconds((end - start)) - TimeUnit.MINUTES
                                .toSeconds(TimeUnit.NANOSECONDS.toMinutes((end - start))));
